...
- `NAMESPACE-MODEL.md` — `kiki:<namespace>/<name>@<version>` identity.
- `MEDIA-TYPES.md` — Kiki-reserved OCI media types.

## Catalog

- `ARTIFACT-CATALOG.md` — canonical Kiki artifacts (the ~30 core ones).
- `META-PACKAGES.md` — desktop, headless, developer, minimal compositions.
- `RELEASE-CADENCE.md` — per-artifact frequency.

## Workflows

- `OCI-WORKFLOWS.md` — build, sign, push (for maintainers).
- `BUILD-SYSTEM.md` — mkosi + cargo + cosign pipeline.
- `SNAPSHOT-PINNING.md` — upstream snapshot strategy.
